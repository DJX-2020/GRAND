...
def train(num_epochs, dataset='cora'):
    ...
    return losses, accs, '{:.4f}%'.format(100 * acc_best), "{:.4f}s".format(t_total)
...
if __name__ == '__main__':
    ...
    for dset in data_list:
        losses, accs, accs_best, t = train(args.epochs, dataset=dset)
        print()
        records.append((dset, accs_best, t))

        # 绘制损失曲线
        plt.figure()
        plt1, = plt.plot(range(0, len(losses)), [ls[0] for ls in losses])
        plt2, = plt.plot(range(0, len(losses)), [ls[1] for ls in losses])
        plt.xlabel('Epochs')
        plt.ylabel('Loss')
        plt.legend([plt1, plt2], ['train_loss', 'val_loss'])
        plt.title('Loss of {}'.format(dset))
        plt.show()

        # 绘制准确度曲线
        plt.figure()
        plt1, = plt.plot(range(0, len(accs)), [a[0] for a in accs])
        plt2, = plt.plot(range(0, len(accs)), [a[1] for a in accs])
        plt3, = plt.plot(range(0, len(accs)), [a[2] for a in accs])
        plt.xlabel('Epochs')
        plt.ylabel('Accuracy')
        plt.legend([plt1, plt2, plt3], ['train_acc', 'val_acc', 'test_acc'])
        plt.title('Accuracy of {}'.format(dset))
        plt.show()
