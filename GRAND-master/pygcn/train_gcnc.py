...
if __name__ == '__main__':
    ...
    for dset in dataset:
        ...
        results.append((dset, test_acc, t))
        print()

        # 绘制损失曲线
        plt.figure()
        plt1, = plt.plot(range(0, args.epochs), [tr[0] for tr in train_rec])
        plt2, = plt.plot(range(0, args.epochs), [vr[0] for vr in val_rec])
        plt.xlabel('Epochs')
        plt.ylabel('Loss')
        ...
        # 绘制准确度曲线
        plt.figure()
        plt1, = plt.plot(range(0, args.epochs), [tr[1] for tr in train_rec])
        plt2, = plt.plot(range(0, args.epochs), [vr[1] for vr in val_rec])
        plt.xlabel('Epochs')
        plt.ylabel('Accuracy')
        plt.legend([plt1, plt2], ['train_acc', 'val_acc'])
        plt.title('Accuracy of {}'.format(dset))
        plt.show()

    print("Final results of GCNC: ", results)
